...
object BinderUtils {
    ...
    /**
     * Check whether the RoamLoc service is started properly
     */
    fun isLocationProviderEnabled(uid: Int): Boolean {
        val packageNames = getUidPackageNames(uid = uid)
        if (uid > 10000 && packageNames?.any {
                !it.contains("com.tsinbei.roamloc")
            } == false) {
            return true
        }
        Logger.warn("Someone try to find RoamLoc: uid = $uid, packageName = ${packageNames?.joinToString()}")
        return uid < 10000
    }
