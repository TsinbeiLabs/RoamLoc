...
import com.tsinbei.roamloc.dobby.Dobby
...
object RemoteCommandHandler {
    ...
    internal val randomKey by lazy { "roamloc_" + Random.nextDouble() }
    private var isLoadedLibrary = false

    @SuppressLint("UnsafeDynamicallyLoadedCode")
    fun handleInstruction(command: String, rely: Bundle): Boolean {
        // Exchange key -> returns a random key -> is used to verify that it is the RoamLoc manager
        ...
        when (commandId) {
            ...
            "load_library" -> {
                ...
                if (isLoadedLibrary && path.endsWith("libroamloc.so")) {
