...
import com.tsinbei.roamloc.xposed.hooks.miui.MiuiBlurLocationProviderHook
import com.tsinbei.roamloc.xposed.hooks.miui.MiuiLocationManagerHook
import com.tsinbei.roamloc.xposed.hooks.telephony.miui.MiuiTelephonyManagerHook
import com.tsinbei.roamloc.xposed.hooks.nmea.LocationNMEAHook
import com.tsinbei.roamloc.xposed.hooks.provider.LocationProviderManagerHook
...
import com.tsinbei.roamloc.xposed.utils.onceHookAllMethod
...
internal object LocationServiceHook: BaseLocationHook() {
    ...
    // A random command is generated to prevent some apps from detecting RoamLoc
...
//        })
        ...
        if(XposedBridge.hookAllMethods(cILocationManager, "registerGnssStatusCallback", object: XC_MethodHook() {
            ...
            XposedBridge.log("[RoamLoc] hook registerGnssStatusCallback failed")
        }
        // android 11+
...
//        })
        ...
        cILocationManager.hookAllMethods("sendExtraCommand", beforeHook {
            ...
            // Not the RoamLoc provider, does not process
            if (provider != "roamloc") {
                if (FakeLoc.enableDebugLog)
            ...
            XposedBridge.hookAllMethods(
                ...
                object : XC_MethodHook() {
                    override fun beforeHookedMethod(param: MethodHookParam?) {
                        ...
                        if (provider == "roamloc" && BinderUtils.isLocationProviderEnabled(userId)) {
                            param.result = true
                        } else if(provider == "network") {
                            param.result = !FakeLoc.enable
                        } else if (FakeLoc.disableFusedLocation && provider == "fused") {
...
//    }
    ...
    private fun addLocationListenerInner(provider: String, listener: IInterface) {
        ...
        if (locationListeners.any { it.first == provider && it.second.asBinder() == binder }) {
            return
        }
...
//
//                        XposedBridge.log("[RoamLoc] ILocationManager.Stub.Proxy: c = ${param.thisObject?.javaClass}, m = ${param.method}")
//                    }
//                })
//            }
//        }
